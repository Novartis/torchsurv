...
$$
\text{Sensitivity}(c) = p(\hat{Y}_i = 1 \mid Y_i = 1) = p(\pi_i > c \mid Y_i = 1),\\
\text{Specificity}(c) = p(\hat{Y}_i = 0 \mid Y_i = 0) = p(\pi_i \leq c \mid Y_i = 0).
$$
...
$$
y(c) = \text{TPR}(c) = p(\pi_i > c \mid Y_i = 1),\\
x(c) = \text{FPR}(c) = p(\pi_i > c \mid Y_i = 0).
$$
...
&= \int_{-\infty}^\infty (1- F_1(c)) f_0(c) \: dc \\
&= \int_{-\infty}^\infty p(\pi_i > c \mid Y_i = 1) p(\pi_j = c \mid Y_j = 0) \: dc \\
&= p(\pi_i > \pi_j \mid  Y_i = 1, Y_j = 0).  \\
\end{align*}
$$
</details>
...
<details>
<summary> Proof of the relation between the C-index and the incident/dynamic AUC </summary>
<br>
