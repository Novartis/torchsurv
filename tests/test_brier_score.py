...
class TestBrierScore(unittest.TestCase):
    ...
    def test_brier_score_real_data(self):
        ...
        for benchmark_brier_score in benchmark_brier_scores:
            ...
            train_time = torch.tensor(benchmark_brier_score["train_time"], dtype=torch.float32)
            test_event = torch.tensor(benchmark_brier_score["test_status"]).bool()
            test_time = torch.tensor(benchmark_brier_score["test_time"], dtype=torch.float32)
            estimate = torch.tensor(benchmark_brier_score["estimate"], dtype=torch.float32)
    ...
    def test_brier_score_compare(self):
        ...
        bs_informative = brier_score_informative(estimate_informative, event, time, new_time, ipcw, ipcw_new_time)[0:-1]
