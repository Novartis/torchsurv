...
get_cindex <- function(TR, TE, train.fit) {
  ...
  c_Harrell_survival <- survival::concordance(
    train.fit,
    newdata = TE
  )$concordance
  ...
  c_Uno_survC1 <- Est.Cval(
    cbind(TE$time, TE$status, lpnew),
    tau = max_times,
    nofit = TRUE
  )$Dhat
  ...
  #
  # survcomp:
  c_survcomp <- survcomp_concordance.index_modified(
    lpnew,
    TE$time,
    TE$status,
    method = "noether"
  )
  c_survcomp_conservative <- survcomp_concordance.index_modified(
    lpnew,
    TE$time,
    TE$status,
    method = "conservative"
...
add_survcomp_ouputs <- function() {
  ...
  body(survcomp_concordance.index_modified)[[46]] <<- substitute(return(list(
    c.index = cindex,
    se = se,
    weights = weights,
    lower = lower,
...
# save metrics
cindexs[[i]] <- get_cindex(TR, TE, train.fit_1)
i <- i + 1
cindexs[[i]] <- get_cindex(TR, TE, train.fit_2)
i <- i + 1
cindexs[[i]] <- get_cindex(TR_complete, TE_complete, train.fit_3)
i <- i + 1
...
cindexs[[i]] <- get_cindex(TR, TE, train.fit_2)
i <- i + 1
cindexs[[i]] <- get_cindex(TR, TE, train.fit_3)
i <- i + 1
