...
get_brier_score <- function(TR, TE, train.fit){
  ...
  for(i in seq_along(times))
    brier_score_survMetrics[i] <- Brier_SurvMetrics_DEBUG(Surv.rsp.new, estimate[,i], times[i])
  ibrier_score_survMetrics <- IBS_SurvMetrics_DEBUG(Surv.rsp.new, estimate, IBSrange = times)


  #
  # SurvAUC

  outputs_survAUC <- predErr(Surv.rsp, Surv.rsp.new, lp, lpnew, times,
...
bugfix_TimeROC_compute_iid_decomposition <- function(){
  ...
  body(Brier_SurvMetrics_DEBUG)[[21]][[4]][[2]][[2]] <<- substitute(time[i] <= t_star & (status[i] == 1))
  print(body(Brier_SurvMetrics_DEBUG)[[21]][[4]][[2]][[2]])

  # controls should be t > t_star instead of t >= t_star
  print(body(Brier_SurvMetrics_DEBUG)[[21]][[4]][[3]][[2]])
  body(Brier_SurvMetrics_DEBUG)[[21]][[4]][[3]][[2]] <<- substitute(time[i] > t_star)
  print(body(Brier_SurvMetrics_DEBUG)[[21]][[4]][[3]][[2]])

  # use the Brier debug version in IBS
  print(body(IBS_SurvMetrics_DEBUG)[[14]][[3]][[2]])
  body(IBS_SurvMetrics_DEBUG)[[14]][[3]][[2]] <<- substitute(bs <- Brier_SurvMetrics_DEBUG(object, sp_matrix, IBSrange))
  print(body(IBS_SurvMetrics_DEBUG)[[14]][[3]][[2]])

  print(body(IBS_SurvMetrics_DEBUG)[[14]][[4]][[4]][[3]][[4]][[4]][[4]])
  body(IBS_SurvMetrics_DEBUG)[[14]][[4]][[4]][[3]][[4]][[4]][[4]] <<- substitute(t_brier[i] <- Brier_SurvMetrics_DEBUG(object, pre_sp, t_star))
  print(body(IBS_SurvMetrics_DEBUG)[[14]][[4]][[4]][[3]][[4]][[4]][[4]])

  print(body(IBS_SurvMetrics_DEBUG)[[14]][[4]][[4]][[4]][[6]][[4]][[4]])
  body(IBS_SurvMetrics_DEBUG)[[14]][[4]][[4]][[4]][[6]][[4]][[4]] <<- substitute(t_brier[i] <- Brier_SurvMetrics_DEBUG(object, pre_sp, t_star))
  print(body(IBS_SurvMetrics_DEBUG)[[14]][[4]][[4]][[4]][[6]][[4]][[4]])
}
...
train.fit_1 <- survreg(Surv(time, status) ~ age, data=TR, dist = 'weibull', scale = 1, x= T, y = T)
train.fit_2 <- survreg(Surv(time, status) ~ age+ sex, data=TR, dist = 'weibull', scale = 1, x= T, y = T)
train.fit_3 <- survreg(Surv(time, status) ~ age+ sex + ph.ecog + ph.karno +
                         pat.karno + meal.cal + wt.loss, data=TR_complete, dist = 'weibull', scale = 1, x= T, y = T)

# save metrics
brier_scores[[i]] <- get_brier_score(TR, TE, train.fit_1); i = i + 1
brier_scores[[i]] <- get_brier_score(TR, TE, train.fit_2); i = i + 1
brier_scores[[i]] <- get_brier_score(TR_complete, TE_complete, train.fit_3); i = i + 1
...
train.fit_1 <-  survreg(Surv(time, status) ~ age, data=TR, dist = 'weibull', scale = 1, x= T, y = T)
train.fit_2 <-  survreg(Surv(time, status) ~ age+ size, data=TR, dist = 'weibull', scale = 1, x= T, y = T)
train.fit_3 <-  survreg(Surv(time, status) ~ age+ size+ grade +
                          nodes + pgr + er + hormon, data=TR, dist = 'weibull', scale = 1, x= T, y = T)

brier_scores[[i]] <- get_brier_score(TR, TE, train.fit_1); i = i + 1
brier_scores[[i]] <- get_brier_score(TR, TE, train.fit_2); i = i + 1
brier_scores[[i]] <- get_brier_score(TR, TE, train.fit_3); i = i + 1
...
write_json(brier_scores, file.path("../benchmark_data/benchmark_brier_score.json"))
