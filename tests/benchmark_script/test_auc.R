...
get_auc <- function(TR, TE, train.fit) {
  ...
  iauc_cd_survAUC <- survAUC::IntAUC(
    AUC_Uno$auc,
    AUC_Uno$times,
    surv.prob,
    max(times),
  ...
  auc_cd_Uno_timeROC <- timeROC_DEBUG(
    T = TE$time,
    delta = TE$status,
    marker = lpnew,
    cause = 1,
  ...
  auc_cd_Uno_se_timeROC <- timeROC_DEBUG(
    T = TE$time,
    delta = TE$status,
    marker = lpnew,
    cause = 1,
  ...
  auc_cd_Uno_riskRegression <- riskRegression::Score(
    list("model" = train.fit),
  ...
  auc_cd_Uno_se_riskRegression <- riskRegression::Score(
    list("model" = train.fit),
    formula = Surv(time, status) ~ 1,
    data = TE,
    cens.method = "ipcw",
  ...
  return(list(
    ...
    times = times,
    surv.prob = surv.prob,
    auc_cd_survAUC = auc_cd_survAUC,
    iauc_cd_survAUC = iauc_cd_survAUC,
    auc_id_sz_survAUC = auc_id_sz_survAUC,
    i_auc_id_sz_survAUC = i_auc_id_sz_survAUC,
...
bugfix_TimeROC_compute_iid_decomposition <- function() {
  ...
  print(body(timeROC_DEBUG)[[41]][[3]][[2]][[4]][[15]][[4]][[2]][[3]][[2]][[
    3
  ]][[1]])
  body(timeROC_DEBUG)[[41]][[3]][[2]][[4]][[15]][[4]][[2]][[3]][[2]][[3]][[
    1
  ]] <<- substitute(compute_iid_decomposition_DEBUG)
  print(body(timeROC_DEBUG)[[41]][[3]][[2]][[4]][[15]][[4]][[2]][[3]][[2]][[
    3
  ]][[1]])

  print(body(compute_iid_decomposition_DEBUG)[[5]][[4]][[2]][[1]])
  body(compute_iid_decomposition_DEBUG)[[5]][[4]][[2]][[1]] <<- substitute(
    compute_iid_decomposition_survival_DEBUG
  )
  print(body(compute_iid_decomposition_DEBUG)[[5]][[4]][[2]][[1]])
...
# save aucs
aucs[[i]] <- get_auc(TR, TE, train.fit_1)
i <- i + 1
aucs[[i]] <- get_auc(TR, TE, train.fit_2)
i <- i + 1
aucs[[i]] <- get_auc(TR_complete, TE_complete, train.fit_3)
i <- i + 1
...
train.fit_3 <- coxph(
  Surv(rfstime, status) ~ age + size + grade + nodes + pgr + er + hormon,
  data = TR,
  method = "efron",
  x = T,
  y = T
...
aucs[[i]] <- get_auc(TR, TE, train.fit_2)
i <- i + 1
aucs[[i]] <- get_auc(TR, TE, train.fit_3)
i <- i + 1
