...
log_likelihoods[[i]] <- get_log_likelihood(
  ...
  data = lung_complete, 
  x = data.frame(rep(1, nrow(lung_complete)),lung_complete[, c('age', 'sex', 'ph.ecog', 'ph.karno', 'pat.karno', 'meal.cal', 'wt.loss')])
); i = i + 1
...
log_likelihoods[[i]] <- get_log_likelihood(
  ...
  data = gbsg, 
  x = data.frame(rep(1, nrow(gbsg)), gbsg[, c('age', 'size', 'grade', 'nodes', 'pgr', 'er', 'hormon')])
); i = i + 1


#
# Save
write_json(log_likelihoods, file.path("../benchmark_data/benchmark_weibull.json"))
